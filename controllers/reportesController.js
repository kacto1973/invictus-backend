...
function cargarImports() {
  // No se utiliza, solo es para que no me borre los imports
  UnidadMedida;
  Gabinete;
  Marca;
  EstadoFisico;
...
async function eliminarPDF(nombre) {
  const oldPath = path.join(__dirname, `../public/pdfs/${nombre}`);
  try {
...
function generarMensajeTendencia(inicio, tendencia, valorAlAzar) {
  if (Math.abs(tendencia) < 0.0005) {
    ...
    return `${inicio} bajan en promedio ${Math.abs(tendencia).toFixed(
      4
    )} al día`;
  }
}
...
async function crearGrafico(opcion) {
  ...
  switch (opcion) {
    case 1: {
      const reactivos = await Reactivo.find({ status: true }).populate(
        "idCategoria"
      );
      ...
      return await chartCanvas.renderToBuffer({
        ...
        data: {
          labels: labels.map(
            (label, i) =>
              `${label} (${data[i]}) (${
                Math.round((data[i] / total) * 10000) / 100
              }%)`
          ),
          datasets: [
            {
        ...
        data: {
          labels: [
            `Entradas (${totalEntradas}) (${
              Math.round((totalEntradas / total) * 10000) / 100
            }%)`,
            `Salidas (${totalSalidas}) (${
              Math.round((totalSalidas / total) * 10000) / 100
            }%)`,
          ],
          datasets: [
            {
    ...
    case 3: {
      ...
      const TotalReactivosAgotados =
        TotalReactivosAgotadosLista[0].totalCantidad;
      const TotalReactivos =
        TotalReactivosLista[0].totalCantidad - TotalReactivosAgotados;
        ...
        data: {
          labels: [
            `Con Stock (${TotalReactivos}) (${
              Math.round((TotalReactivos / total) * 10000) / 100
            }%)`,
            `Sin Stock (${TotalReactivosAgotados}) (${
              Math.round((TotalReactivosAgotados / total) * 10000) / 100
            }%)`,
          ],
          datasets: [
            {
    ...
    case 4: {
      ...
      const mensajeTendencia = generarMensajeTendencia(
        "Las Reservas",
        (tendenciaData[tendenciaData.length - 1].y - tendenciaData[0].y) / 30,
        tendenciaData[0].y
      );
    ...
    case 1: {
      ...
      reactivos.forEach((rea) => {
        datos.push([
          ...
          rea.unidadMedida.valor.toString() +
            rea.unidadMedida.idUnidadMedida.nombre,
    ...
    case 3: {
      ...
      equipos.forEach((equ) => {
        datos.push([
          equ.nombre,
          equ.descripcion,
          equ.requiereMantenimiento === true
            ? "Si"
            : equ.requiereMantenimiento === false
            ? "No"
            : "ND",
        ]);
      });
      ...
      return {
        ...
        headers: ["Nombre", "Descripcion", "¿Requiere Servicio?"],
        rows: datos,
        options: {
          columnSpacing: 3,
          padding: 3,
    ...
    case 5: {
      ...
      reservas.forEach((rev) => {
        ...
        datos.push([
          ...
          vencido === true ? "Vigente" : vencido === false ? "Vencido" : "ND",
        ]);
      });
...
async function generarNombreUnico() {
  ...
  const nombresSinExtension = reportesSupa
    .filter((fname) => fname.toLowerCase().endsWith(".pdf"))
    .map((fname) => fname.replace(/\.pdf$/i, ""));
...
const crearReporte = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      res
        .status(400)
        .json({ error: "Error en la seleccion de opciones del reporte." });
      return;
    }
    ...
    const idReporte = await crearEntradaReporte(
      "Reporte " + nombrePDF,
      "No hay url todavia :c",
      "En proceso"
    );
    ...
    const insertarEncabezado = (
      tituloReporte = "Nombre del reporte",
      firstPage
    ) => {
      if (firstPage) {
        ...
        doc.rect(0, 0, pageWidth, headerHeight).fill("#DBACFD");

        const logoBoxWidth = 120;
        doc
          .rect(pageWidth - logoBoxWidth, 0, logoBoxWidth, headerHeight)
          .fill("#CA88FC");
        ...
        doc
          ...
          .fillColor("white")
          .fontSize(18)
          .text("Reporte del Día", 50, 20, {
            // <-- cambiamos de 35 a 20
            width: textBoxWidth,
            align: "left",
          });
    ...
    doc.on("pageAdded", () =>
      insertarEncabezado("Reporte " + nombrePDF, false)
    );
    ...
    doc.end();
    const targetPath = path.join(__dirname, `../public/pdfs/${nombrePDF}.pdf`);
    ...
    try {
      await fs.promises.writeFile(targetPath, doc);
    } catch (err) {
      console.error("❌ Error al subir:", err);
      await crearEntradaReporte(
        nombrePDF,
        "No hay url todavia :c",
        "Error",
        idReporte._id
    ...
    const url = `/pdfs/${nombrePDF}.pdf`;

    const verificacionEntrada = await crearEntradaReporte(
      nombrePDF,
      url,
      "Completado",
      idReporte._id
    ...
    if (verificacionEntrada.error) {
      console.error("Error al crear la entrada del reporte:", error);
      const verificacionEliminar = await eliminarPDF(nombrePDF);

      if (verificacionEliminar.error) {
        console.error("Error al eliminar el PDF:", verificacionEliminar.error);
        res.status(500).json({
          error: "Error al crear la entrada del reporte y al eliminar el PDF",
        });
        return;
      }
...
const eliminarReporte = async (req, res) => {
  ...
  try {
    ...
    const verificacionSupa = await eliminarPDF(fileName);
...
const cambiarNombreReporte = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Nombre del reporte actualizado exitosamente" });
  } catch (error) {
    console.error(error);
...
const conseguirReportesPorRangoDeFechas = async (req, res) => {
  ...
  try {
    ...
    const fechaInicioDate = dayjs
      .tz(fechaInicio, "America/Hermosillo")
      .startOf("day")
      .toDate();
    const fechaFinDate = dayjs
      .tz(fechaFin, "America/Hermosillo")
      .endOf("day")
      .toDate();
